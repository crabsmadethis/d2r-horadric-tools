...
| Bind Demon level behavior | `Skills.txt` grants Extra Strong at level 5, Extra Fast at 10, Spectral Hit at 15, and Aura Enchanted at 20 through Bind Demon calc fields. Fresh natural binds at hard levels 1, 5, 10, and 20 matched those thresholds, while payload `+52` stayed `7` for each completed bind. | Fill the missing level-15 natural bind, then update writer/scanner terminology so `+52` is no longer treated as effective skill level. |
