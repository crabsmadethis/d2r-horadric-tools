...
| Bound-demon visible affixes | Direct Spectral Hit and Aura Enchanted bytes can persist without visibly displaying those properties. A pre-authored empty-affix follower matrix at hard Bind Demon levels 1, 5, 10, 15, and 20 displayed no added properties. | Treat the five MonUMod bytes, hard skill level, and payload `+52` as persisted inputs, not the whole visible-property model. |
...
| Bind Demon level behavior | Payload `+52` persists, but it is not enough to add visible properties to a pre-authored follower. A natural bind with the Warlock skill block at Bind Demon level 20 also saved `+52 == 7`, so this may not be the effective skill level. | Use fresh natural binds for skill-tier behavior; do not infer it from pre-authored follower payloads. |
